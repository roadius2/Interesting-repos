...
- **[SenteLabsAI/OpenExecutive](https://github.com/SenteLabsAI/OpenExecutive)** — A virtual executive team: eight specialist agents (CSO, CFO, CHRO, GC, COO, CMO, CPO, board director) behind an orchestrator that presents as one coherent voice, so the agent architecture never leaks to the user. RAG over your own company docs plus episodic memory in SQLite, reachable from Slack/Discord/Telegram/email, with spend thresholds and audit logging for governance. FastAPI + Next.js, and it ships an eval suite of 29 scenarios scored LLM-as-judge.
